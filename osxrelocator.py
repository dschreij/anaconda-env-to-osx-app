...
class OSXRelocator(object):
    ...
    def change_id(self, object_file, id=None):
        id = id or object_file.replace(self.lib_prefix.decode('utf-8'), 
            self.new_lib_prefix.decode('utf-8'))
    ...
    def change_libs_path(self, object_file):
        library_list = self.list_shared_libraries(object_file)
        # Check ID first
        ID = library_list[0]
        if self.lib_prefix in ID:
            new_ID = ID.replace(self.lib_prefix, self.new_lib_prefix)
            print("{}: Changing ID: {} to {}".format(os.path.basename(object_file), 
                ID, new_ID))
            self.change_id(object_file, new_ID)

        for lib in library_list:
            if self.lib_prefix in lib:
                new_lib = lib.replace(self.lib_prefix, self.new_lib_prefix)
                print("{}: Changing lib reference {} to {}".format(
                    os.path.basename(object_file), lib, new_lib))
